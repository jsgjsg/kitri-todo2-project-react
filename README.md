...
- Jira, excalidraw, SourceTree를 처음 사용해봄
  - 개발 첫 날에 협업이 매끄럽지 않았다.

- 24.06.24 65일차
    - Jira 가입 ⇒ 팀원들과 일정 관리
    - excalidraw ⇒ UI 설계
    - github, SourceTree ⇒ 코드 버전 관리
    - express, react 간단한 연결 test 완료
- 24.06.25 66일차
    - 로그인, 회원가입, 정보수정 등 코드 수정
        1. username ⇒ email로 변경
    - todo list 등록, 수정, 삭제 코드 수정
        1. 변수명이 기존에는 id였는데, mongoDB와 연결하면서 _id가 되었고, 해당 부분에서 오류가 발생했다는 것을 발견 및 수정 완료
        2. 등록, 수정 및 삭제 완료 시 res.send(”작업 완료”)를 보내서 오류가 발생한 것을 발견 및 수정
    - 어려웠던 점
        - 프론트와 백의 분업이 원활하지 않음
            - 중간중간 테스트를 하려고 해서?
- 24.06.26 67일차
    - 조금 더 협업을 매끄럽게 할 수 있는 방법을 생각해봄
        - 시작 전 excalidraw에서 전체 진행 흐름을 그려보았다.
        
        ![Untitled](https://prod-files-secure.s3.us-west-2.amazonaws.com/b3e11e83-005d-455f-8ab3-4e5570c337a1/d1240f0f-e693-433e-b9b5-4f1176998500/Untitled.png)
        
        - 전체 흐름을 보며, React와 Express가 각각 해야할 일을 작성하였다.
            - 작성한 것들은 Jira에 옮겨 적었고, 번거롭지만 팀원들과 Jira를 활용하여 협업하려고 노력하였다.
    - 기한 있는 todolist를 추가하였다.(deadlineTodolist)
        - 해당 투두리스트의 DB를 생성하였다.
        - 해당 투두리스트의 CRUD endpoint를 생성하였다.
        - 달력에 todolist가 있는 날짜를 표시하기 위해 “Todolist가 있는 날짜를 반환”하는 endpoint를 생성하였다.
    
    - 느낀 점
        - excalidraw에 흐름을 그려보니, 해야할 작업들이 조금 더 명확하게 보였다.
        - Jira를 활용하는 것이 처음에는 번거로웠는데, 하다보니 팀원들간 대화를 하지 않고도 진행 상황을 어느정도 파악할 수 있었다.
- 24.06.27 68일차
    - 프로젝트 개발을 재개하기 전
        - excalidraw를 보고, 실제 구현되어있는 상황을 테스트하며  어떤 기능들을 추가해야 하는지 상기하고, 수정하였다.
    - endpoint가 겹치는 상황 발생
        - 겹쳤던 endpoint : `/api/todos/:id` , `/api/todos/:date` , `/api/todos/hasDate`
            - 앞 2개는 변수를 각각 다르게 받기에 별 생각 없었고, `/hasDate`는 “hasDate”를 받기 때문에 전혀 겹치지 않는다고 생각함.
            - 해당 endpoint들을 사용할 때 오류가 발생해서 생각해보니, 예전에도 겪었던 상황이 떠올라서 각 endpoint를 `/api/todos/id/:id`, `/api/todos/date/:date` 로 수정(마지막껀 수정X)하였다.
    - 깃허브 충돌 발생
        - 어제까지는 같은 폴더를 작업 시 merge를 하지 않고 pull을 해서 오류가 발생하지 않았었음
            - 코드가 갑자기 날아가서 고민해보니, merge를 하지 않았던 것을 알았다.
        - 오늘 로컬의 branch에서 merge해보니 충돌이 발생했고, 코드를 합치는 것이 조금 까다로웠다.
        - 같은 폴더에서 작업하더라도, 되도록 같은 파일은 건드리지 않아야겠다는 생각이 들었다.
- 느낀 점
    
    endpoint의 중복, 프론트와 백 사이의 변수가 다른 문제 등을 겪으니 구현 전에 설계를 잘 했으면 좋았을 것 같다고 느꼈다.
